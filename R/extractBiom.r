...
extractBiom <- function(stat, tdr, biomdir) {
	...
	statVars <- userHeader(c("Lat", "Lon", "Date", "Pixel.id"), type="stat")
	findVars(statVars, stat)
	tdrVars <- userHeader(c("Layer", "is.Day", "Pixel.id", "Date"), type="tdr")
	findVars(tdrVars, tdr, varnames=c("tdrLayer", "tdris.Day", "tdrPixel.id", "tdrDate"))
